...
public class GrandpaDialogue2 : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if ((playerInRange && Input.GetKeyDown(KeyCode.Return)) && ((GameManager.Instance.spokeToCousin1 || GameManager.Instance.spokeToCousin2)) && (!ConversationManager.Instance.IsConversationActive))
        {
            ...
            {
                Debug.Log("Starting granpa conversation (not spoken to cousin2)");
            }
            else if (GameManager.Instance.spokeToCousin2)
            {
                Debug.Log("Starting granpda conversation (spoken to cousin2)");
            }
